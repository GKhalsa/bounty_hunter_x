...
function userCollisionDetection(){
  enemyMissileArray.forEach(function(missile){
    if (missile.x > shipX && missile.x < shipX + shipWidth && missile.y > shipY && missile.y < shipY + shipHeight){console.log('hi');}


    // detection top left= missile.x >




    //bottom of missile is (bottom_left: x, y + missileHeight, bottom_right: x + missileWidth, y + missileHeight
    //top of player is (top left: x,y top rigt:x + playerwidth,y)
    //left missile (top left:x,y, bottom left: x, y + missile height)
    //right player (topright: x + sihpwidth,y, bottomright: x + shipwidth, y + shipheight)
