...
GameBoard.prototype.generateEnemyPlanes = function(){
  for(let i = 0; i< 5;i++){
    ...
    this.enemyPlanes.push(this.enemyPlaneSetsLeft(enemies.x, enemies.y, enemies.movement ));
  }
};
...
GameBoard.prototype.enemyPlaneSetsLeft = function(x,y,movement){
  return new EnemyPlane(x, y, 30, 50, this.ctx, this.canvas, movement);
};

GameBoard.prototype.checkEnemyPlaneLocations = function(){
 return this.enemyPlanes.every(plane => plane.x > this.canvasWidth);
};
